...
## 2026-08-28 (later still) — The Spanish explanation, and the fourth GBNF lesson

Notice 06 is a Spanish SAR 7. The model explained it in fluent English.

The first attempt at a fix was to stop asking the model to infer the language and
tell it instead: `Notice.locale` is stored at save time, so the system turn now
ends with a literal `Reply in Spanish.` rather than *"Reply in the same language
as the letter."*

**It did not work.** Re-run on the phone, told `es`, and the answer came back in
English again. English did not regress — notice 01 improved, and now names all
three required documents — but the Spanish notice is still answered in the wrong
language.

### Why, and it is the same lesson a fourth time

The grammar hard-codes the section labels:
...
Those are literal English tokens. **The grammar forces the model to begin all
three lines in English**, and having been made to write `SAYS: ` it continues in
the language it has just been compelled into. The system instruction and the
grammar are in direct conflict, and the grammar wins, because the grammar is
enforced at the token level and the instruction is only a request.

```
no null production   ->  fabricated the missing value
digit ban            ->  wrote numbers as words, and looped
structural grammar   ->  satisfied by an echo of the template itself
English labels       ->  forced the answer into English
```

A grammar constrains shape. **Shape carries language**, and that had not been
noticed because every previous test was in English, where a constraint that
forces English is invisible.

### Not fixed here, deliberately

The fix is to make the labels language-dependent — a Spanish grammar emitting
`DICE: `, `HAGA: `, `APELAR: ` — and `parseSections` keys on the same literals,
so both the grammar and the parser change together, and `explain-check.ts` runs
on the result. That is a change to the shape of a guardrail, not a tweak, and the
instruction for this session was to stop on the model once the output was right.
Recorded rather than attempted.

**The interim state is honest**: an English explanation of a Spanish letter is
wrong for the user this app exists for, so the feature should not be demonstrated
on a Spanish notice until the labels are fixed.

### What the metrics could not see

Every number said this was working. 33 tok/s, clean EOS, grammar satisfied, three
sections, sanity pass green, content accurate. **The measurement was blind to the
one property that mattered most for the person the app is built for**, because it
measured whether text was produced and not whether that text could be read by its
reader.

That is the same shape as the accented-names finding, and it is worth stating as
a pair:

> `recipient_name` scored near its ceiling on a corpus with no accented names.
> The explanation scored perfectly on a Spanish notice answered in English.
>
> **A measurement inherits the blind spots of whoever chose what to measure**,
> and both times the blind spot fell on the Spanish-speaking user — not by
> anyone's intent, but because the person writing the test wrote it in English
> and the corpus was generated by someone who typed what they could type.

---

## 2026-08-28 — A browser UI workbench, and the SPEC §10 line it runs along

The real screens now render in a browser beside the editor, with fast refresh,
so layout and copy can be worked on without a rebuild. `npm run web`, then
`http://localhost:8083`.

**This sits next to a scope guard and the tension is worth writing down.**
SPEC §10 says "❌ A web version", and `DEPENDENCIES.md` listed
`react-native-web` and `react-dom` under *deliberately not used* for exactly
that reason. What was built is a development tool, not a web version of the
product — the same category as `tools/metrics/` or the Simulator — but the line
is close enough that it was kept as far from the product as it could be:

- The three packages are **devDependencies**, not dependencies.
- **`app.json` is untouched.** The web block that `expo start --web` normally
  wants turned out to be unnecessary under SDK 57, so the app manifest declares
  no web platform and no web build exists.
- The stub swap in `metro.config.js` is keyed on `platform === 'web'`.
- `src/lib/dev-seed.ts` is a **no-op** that the native bundle resolves instead of
  `dev-seed.web.ts`.

**Verified rather than assumed:** the iOS bundle was fetched from the same Metro
and searched. `web-preview` appears **0** times; `SAMUEL BRIGHT`, `CARMEN
DELGADO` and `seedVersion` — strings unique to the seeder — appear **0** times;
`llama.rn`, `expo-mlkit-ocr` and `datetimepicker` are all still the real
modules. The one `MARIA REYES` hit in the iOS bundle is from
`src/extraction/name.ts`, which is app code. This is the fourth time "configuring
a thing is not verifying it happened" would have caught something, so the
artifact was read.

### What it is and is not

**Real:** every screen and component, the theme tokens, i18n in both languages,
routing, the countdown and urgency logic, the checklist rules, the content packs
— and a genuine database, because `expo-sqlite` ships a WASM build and runs its
actual code rather than a hand-written fake.

**Stubbed:** camera, OCR, the model, notifications, keychain, filesystem.

The database being real matters more than it sounds: the seeder writes through
`saveNotice`, so redaction refusal, case-number hashing and field encryption all
execute exactly as they do on the phone.

**The rule that keeps it honest:** a screen that looks right in the browser has
been shown to *lay out* right and nothing more. Behaviour is still proven on the
phone. Never write "verified on web" — the same rule already in force for
"device" versus "Simulator", which was swept for five wrong claims on 2026-08-26.

### Two things it cost, both worth having

**A false defect, caught before it was reported.** Home rendered `calfresh` and
`medi_cal` where it should say CalFresh and Medi-Cal, which looked like a display
bug in `index.tsx`. It was the seed: the real extractor emits display names —
`ground_truth.json` has `CalFresh`, `Medi-Cal`, `CalFresh/CalWORKs` — so
`notice.programId` is already display-ready and the screen is correct. Checking
what the pipeline actually produces took two minutes and would otherwise have
been a wrong bug report.

**A seed that would not update.** The corrected names did not appear on reload,
because the database survives a reload and the seeder skipped when rows already
existed — so an edit to the file looked like it did nothing. `SEED_VERSION` now
forces a wipe and reseed when it changes. Same shape as every caching bug in this
repo: the guard was asking "is there data?" when the question was "is the data
current?".
