...
| `react-native-web`, `react-dom`, `@expo/metro-runtime` | `dev/web-preview/`, `metro.config.js` | **devDependencies.** Render the real screens in a browser for layout and copy work, with fast refresh — see `dev/web-preview/README.md`. Not a platform target: the native leaves are stubbed, the swap is keyed on `platform === 'web'`, and the iOS bundle was checked byte-wise to confirm none of it is reachable. Deleted before the freeze with `src/lib/diagnostics/`. |
...
| `react-native-web`, `react-dom` **as a shipping target** | SPEC §10 forbids a web version, and that still holds: there is no web build, no web output in any release, and `app.json` declares no web platform. They are installed as **devDependencies** for `dev/web-preview/` — a UI workbench that renders the real screens in a browser beside the editor. See the row in the dev-tooling table and NOTES.md 2026-08-28. |
